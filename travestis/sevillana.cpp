#include <iostream>
using namespace std;
int main(){
	cout << "A la Ana la Sevillana, a la Ana la Sevillana," << endl;
	cout << "no me da miedo de ti maricón, de nunca," << endl;
	cout << "a la Ana la Sevilla cuando fui a tu casa " << endl;
	cout << "para que a mi ex novio el italiano le pusieras el mentón" << endl;
	cout << "que me cobraste 33 mil pesetas" << endl;
	cout << "la tenias tumbá en el sofá allí con tres litros" << endl;
	return 0;
}